...
[![Tests](https://img.shields.io/badge/tests-936%20passed-brightgreen)](https://github.com/SimonBouhier/EPP_Verdict/tree/main/tests/)
[![ADRs](https://img.shields.io/badge/ADRs-22-blue)](https://github.com/SimonBouhier/EPP_Verdict/tree/main/docs/adr/)
...
The full liability framing — and the alignment with UNESCO §3/§5/§11 plus the emerging legal recognition of blockchain evidence (Tribunal judiciaire de Marseille 2025, CJUE *DigitalArt GmbH* 2024, Luxembourg Code civil 2024) — is in [`WHITEPAPER.md` → Liability & Scope](/whitepaper/#liability--scope).
...
The merge records acceptance into the project registry, not truth. The epistemic tier stays distinct from GitHub's promotion authority. See [ADR-021](/adrs/adr-021-github-governance/).
...
**For the full architectural and epistemological narrative** → [`WHITEPAPER.md`](/whitepaper/).
**For the 3-minute pitch** → [`PITCH.md`](/pitch/).
...
**Full component-by-component map** → [`docs/ARCHITECTURE.md`](/architecture/).
...
| On-chain attestations | **12** pushed ([data/devnet_pushed.json](https://github.com/SimonBouhier/EPP_Verdict/blob/main/data/devnet_pushed.json)) |
| Flywheel delta demonstrated | **+0.46** (0.43 → 0.89) |
| Formal verification | Lean 4 — **6 substantive theorems** (4 tier `iff` characterization + 2 stratification cumulativity) + 7 regression tests + 2 type-level invariants (`Option SourceAnchor`) — see ADR-020 + audit P1–P4 in [`docs/audit/`](https://github.com/SimonBouhier/EPP_Verdict/tree/main/docs/audit/) |
...
- **[`WHITEPAPER.md`](/whitepaper/)** — Full architectural and epistemological narrative: ESMM deliberation, metrological frames, flywheel, formal verification, cluster vision, references.
- **[`PITCH.md`](/pitch/)** — 3-minute pitch: three acts, three primitives nobody implements, the five axioms, the verdict that survives the counterpoint stress-test.
- **[`docs/ARCHITECTURE.md`](/architecture/)** — Living component-by-component map.
- **[`docs/adr/`](https://github.com/SimonBouhier/EPP_Verdict/tree/main/docs/adr/)** — 21 Architecture Decision Records.
- **[`docs/positioning/`](https://github.com/SimonBouhier/EPP_Verdict/tree/main/docs/positioning/)** — Internal strategic material (competitive scan, counterpoint responses, formal methods landscape, track strategy, *the negative space* essay). Not public vitrine — source material informing the public docs.
- **[`docs/fr/CHANGELOG.md`](/changelog/)** — Chronological journal of significant changes.
