"""Versioned metrological frames used by the EPP core.

A frame defines what is measured and how for a specific domain. Its
canonical SHA-256 hash identifies the exact methodology independently of
the persistence or promotion backend. Frames are stored locally in SQLite
and may be published as Git artifacts. Optional adapters, including Solana,
can project the same hash into their own representation.
"""
...
class FrameGovernance(BaseModel):
    """Legacy methodology-governance metadata included in the frame hash.

    These fields describe the historical amendment target of a methodology;
    they do not grant promotion authority. ADR-021 assigns artifact promotion
    to protected Git merges. Defaults stay unchanged to preserve published
    frame hashes.
    """
...
class MetrologicalFrame(BaseModel):
    ...
    def compute_frame_hash(self) -> str:
        ...
        et retourne le hash hex. Les adaptateurs externes peuvent publier
        cette valeur sans devenir une dependance du noyau metrologique.
        """
