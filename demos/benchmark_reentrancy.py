...
Pré-requis:
    - Ollama running avec mistral:latest, deepseek-r1:latest, gemma3:latest
...
def print_unit_result(unit_name: str, severity: str, consensus: float, 
    ...
    print(f"  {icon} [{severity.upper():15s}] {unit_name}()")
    ...
    if unit_name in GROUND_TRUTH:
        ...
        match = "✅ MATCH" if severity == gt["severity"] else f"⚠️ EXPECTED {gt['severity']}"
...
def print_slither_summary(slither_data: dict) -> None:
    ...
    print()
    if reentrancy_found:
        print("  ✅ Slither confirms SWC-107 (reentrancy-eth) on withdrawBalance")
    else:
        print("  ⚠️ Slither did NOT detect reentrancy-eth")
    print()
...
async def run_epp_audit(contract_path: str, dry_run: bool = False) -> dict:
    ...
    for unit in slice_result.units:
        ...
        print(f"  [{priority}] {unit.unit_name}() — ext:{unit.external_calls} sw:{unit.state_writes}")
    ...
    models = MODELS
    if dry_run:
        print("  ⚠️ DRY-RUN MODE — MockProvider (no Ollama)")
    ...
    # TODO: Claude Code — vérifier que run_audit() accepte ces paramètres
    # et ajuster si l'interface a évolué depuis le plan.
...
async def main():
    ...
    if slither_data and epp_results:
        ...
        if slither_has_reentrancy and epp_severity in ("high", "medium"):
            print("  ✅ CONCORDANT — Both Slither and EPP flag high-severity reentrancy")
        elif slither_has_reentrancy and epp_severity == "informational":
            print("  ⚠️ DISCORDANT — Slither flags reentrancy, EPP says informational")
        elif not slither_has_reentrancy and epp_severity in ("high", "medium"):
            print("  ⚠️ DISCORDANT — EPP flags vulnerability, Slither missed it")
        else:
