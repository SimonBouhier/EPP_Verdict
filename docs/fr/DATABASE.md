...
### Pool de Connexions (v2.1.0)

L'engine utilise maintenant un pool de connexions SQLite:

```python
# database/engine.py - Initialisation automatique
...
|-----------|--------|-------------|
| `pool_size` | 10 | Connexions pre-allouees |
| `max_overflow` | 5 | Connexions supplementaires si surcharge |
| `connection_timeout` | 30s | Timeout acquisition |
| `idle_timeout` | 300s | Fermeture connexions inactives |
...
PRAGMA busy_timeout=30000;    -- 30s attente sur contention (NOUVEAU v2.1.0)
```

**Note importante:** Le `busy_timeout` evite les echecs immediats lors de contention. Sans ce pragma, une requete echoue instantanement si la base est verrouillee.
...
await close_db()  # Ferme le pool ET libere les ressources
```
